...
const VisitArtirstProfile = props => {
  const {
    feedItem,
    profileDetails,
  ...
  const [isTabsbar, setTabsBar] = useState(isFloating);
  const [screenHeightOnScroll, setScreenHeightOnScroll] = useState(() => false);
  useEffect(() => {
    if (!scrollAnimated) {
      return;
    }
    const listenerId = scrollAnimated.addListener(a => {
      isTransparent !== a.value < 150 && setTransparent(!isTransparent);

      isTabsbar !== a.value < 150 && setTabsBar(!isTabsbar);
    });
  ...
  useEffect(() => {
    mixpanel.track('Visit', {
      PageName: 'Artist Visiting Profile',
  ...
  const renderPinnedPostsAndCollectionsList = () => {
    return (
      <FlatList
  ...
  const renderPinToCollectionList = () => {
    return (
      <FlatList
        numColumns={3}
        ...
        renderItem={({item}) => (
          <CollectionItem
            ...
            onItemPress={() => {
              ...
              mixpanel.track('Visit', {
                PageName: 'Visit Artist Pin To Collection',
        ...
        <Animated.ScrollView
          ...
          <ImageBackground
            ...
            <UserProfileComponent
              ...
              rightImageOnPress={() => setIsMenuActionSheetVisible(true)}
              rightImage={Images.threeHorizontalDots}
              leftImage={Images.backButton}
              notificationCount={0}
            />
            {renderTabBar()}
          </ImageBackground>
...
export default connect(mapStateToProps)(VisitArtirstProfile);
